// server-vercel.js - Entry point for Vercel deployment (serverless version)
...
// Use MemoryStore instead of PgSession for serverless environments
...
app.use(session({
  store: sessionStore,
  secret: process.env.SESSION_SECRET || 'game-spotlight-secret-key-' + Math.random().toString(36).substring(2, 15),
  resave: false,
...
app.get('/api/games/trending', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error fetching trending games', error: error.message });
  }
});
...
app.get('/', (req, res) => {
  res.status(200).json({
    status: 'ok',
    message: 'Game Spotlight API Server is running',
    timestamp: new Date().toISOString(),
    environment: process.env.NODE_ENV || 'development',
    database: 'connected' // This is optimistic - we should check the connection
  });
});
...
app.get('/health', async (req, res) => {
  try {
    ...
    const dbConnected = dbTest && dbTest.rows && dbTest.rows.length > 0;
...
// DB TEST ENDPOINT (for debugging database issues)
app.get('/api/db-test', async (req, res) => {
  try {
    ...
    try {
      ...
      usersTable = { exists: true, count: usersResult.rows[0].count };
    } catch (err) {
      usersTable = { exists: false, error: err.message };
    }
    ...
    try {
      ...
      gamesTable = { exists: true, count: gamesResult.rows[0].count };
    } catch (err) {
      gamesTable = { exists: false, error: err.message };
    }
    ...
    res.json({
      ...
      database: {
        connected: true,
        time: testQuery.rows[0].time,
        tables: {
          users: usersTable,
          games: gamesTable
        },
        connectionString: `${process.env.PGHOST}:${process.env.PGPORT}/${process.env.PGDATABASE} (user: ${process.env.PGUSER})`,
...
// For regular Node environments (testing and development)
if (process.env.NODE_ENV !== 'production' || process.env.IS_LOCAL_DEV === 'true') {
